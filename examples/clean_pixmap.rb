...
values.write_array_of_uint32([white_pixel, event_mask])
...
gc_pixmap_id = XCB.xcb_generate_id(conn)
puts "✅ ID GC для пиксмапа: #{gc_pixmap_id}"

# Создание GC с черным цветом для пиксмапа
gc_values = FFI::MemoryPointer.new(:uint32, 1)
gc_values.write_array_of_uint32([black_pixel])

XCB.xcb_create_gc(conn, gc_pixmap_id, pixmap_id, XCB::XCB_GC_FOREGROUND, gc_values)
puts "✅ Создан GC с черным цветом для пиксмапа"

# Очистка пиксмапа белым цветом
XCB.xcb_clear_area(conn, 1, pixmap_id, 0, 0, 200, 200)
puts "✅ Очищен пиксмап белым цветом"
...
XCB.xcb_poly_rectangle(conn, pixmap_id, gc_pixmap_id, 1, rect)
puts "✅ Нарисован черный прямоугольник в пиксмапе"

# Создание графического контекста для окна
gc_window_id = XCB.xcb_generate_id(conn)
puts "✅ ID GC для окна: #{gc_window_id}"

# Создание GC для окна (без особых настроек)
XCB.xcb_create_gc(conn, gc_window_id, window_id, 0, nil)
...
loop do
  ...
  when XCB::XCB_EXPOSE
    ...
    XCB.xcb_copy_area(conn, pixmap_id, window_id, gc_window_id, 0, 0, 0, 0, 200, 200)
    XCB.xcb_flush(conn)
...
XCB.xcb_free_gc(conn, gc_pixmap_id)
XCB.xcb_free_gc(conn, gc_window_id)
